...
def get_bgm_path(category=None):
    resource_path = get_resource_base_dir_path()
    bgm_base_path = os.path.join(resource_path, 'audio','bgm')
    if category is None:
        return bgm_base_path
    if category in Category.DEFINED_CATEGORIES:
        bgm_catg_path =  os.path.join(bgm_base_path, category)
        print(bgm_catg_path)
        for item in os.listdir(bgm_catg_path):
            if item.endswith('.mp3') or item.endswith('.wav'):
                bgm_file = os.path.join(bgm_catg_path, item)
                print(bgm_catg_path)
                return bgm_file
        if str(category) == Category.ALL:
            raise Exception(' get_bgm_path :: No audio file found in directory:  ' + bgm_catg_path )
        else:
            return get_bgm_path(Category.ALL)
    else:
        raise Exception(' get_bgm_path :: Category parameter is incorrect :: "' + category + '" not supported')
