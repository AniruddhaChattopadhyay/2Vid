...
def getNewsFromUrl(url,data=None)->NewsIdResponse:
...
def getNews(category, number_of_pages_to_crawl):
    ...
    response = parse_and_write_to_db(request.text, category)
    ...
    while (page_number <= number_of_pages_to_crawl):
        ...
        try:
            ...
            print(e.message)
            return
        response = parse_and_write_to_db(json.loads(request.text)['html'], category)
        news_count+=response['news_count']
