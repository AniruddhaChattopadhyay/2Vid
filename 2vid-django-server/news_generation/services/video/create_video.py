...
from newsX_backend.utils.directory_helper import get_news_path, get_audio_file_path, get_news_images_path, get_news_video_path, get_news_video_lang_path, get_zoom_videos_path, get_intro_video_path, get_outro_video_path, get_bgm_path, get_transition_audio_path
from newsX_backend.utils.db_utils import get_language_by_title, get_raw_news_by_id, insert_video_link
from newsX_backend.utils.firebase_utils import UploadVideo
from newsX_backend.enums.model_enums import VideoLinksEnum, RawNewsEnum
...
from newsX_backend.mappers.category_types import Category
...
BGM_FLAG = config.getboolean('VIDEO','VIDEO_BGM_FLAG')
...
def create_video(news_dict, language):
    ...
    if news_dict[RawNewsEnum.source.value] == "YES":
        bgm_audio = editor.AudioFileClip(get_bgm_path(category=Category.ALL))
    ...
    elif news_dict[RawNewsEnum.source.value] == "NO":
        pass
    elif BGM_FLAG:
        bgm_audio = editor.AudioFileClip(get_bgm_path(category=Category.ALL))
        bgm_audio = bgm_audio.subclip(0, audio.duration)
