...
def news_generation_by_url(request):
    ...
    try:
        data = request.data
        url = data['url']
        async_response = get_news_from_url_task.delay(url)
        response = {"request_data": data }
        return Response(response, status=status.HTTP_200_OK)
    except Exception as error:
...
def process_news_from_url(request):
    ...
    try:
        ...
        chain_task = chain(get_news_from_url_task.s(url), translate_news_task.s(languages), tts_task.s(languages), video_generation_task.s(languages), send_mail_task.s(languages)).delay()
...
def script_to_video(request):
    ...
    try:
        ...
        return internal_server_error(str(e.with_traceback()))
